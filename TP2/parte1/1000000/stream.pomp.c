...
{   POMP2_Parallel_begin( &pomp2_region_1 );
    {
#pragma omp master
{   POMP2_Master_begin( &pomp2_region_2, pomp2_ctc_2  );
	{
	    k = omp_get_num_threads();
	    printf ("Number of Threads requested = %i\n",k);
        }
  POMP2_Master_end( &pomp2_region_2 ); }
    }
{ POMP2_Task_handle pomp2_old_task;
...
#pragma omp parallel     POMP2_DLIST_00006 firstprivate(pomp2_old_task) if(pomp2_if) num_threads(pomp2_num_threads)
...
#pragma omp parallel     POMP2_DLIST_00008 firstprivate(pomp2_old_task) if(pomp2_if) num_threads(pomp2_num_threads)
...
{   POMP2_For_enter( &pomp2_region_9, pomp2_ctc_9  );
#pragma omp          for                   nowait
	for (j=0; j<STREAM_ARRAY_SIZE; j++)
	    c[j] = a[j]+b[j];
{ POMP2_Task_handle pomp2_old_task;
...
#else
{
  ...
  int pomp2_if = 1;
  POMP2_Task_handle pomp2_old_task;
  POMP2_Parallel_fork(&pomp2_region_10, pomp2_if, pomp2_num_threads, &pomp2_old_task, pomp2_ctc_10 );
#pragma omp parallel     POMP2_DLIST_00010 firstprivate(pomp2_old_task) if(pomp2_if) num_threads(pomp2_num_threads)
{   POMP2_Parallel_begin( &pomp2_region_10 );
...
#pragma omp barrier
  ...
  POMP2_Parallel_end( &pomp2_region_10 ); }
  POMP2_Parallel_join( &pomp2_region_10, pomp2_old_task ); }
#endif
...
{   POMP2_For_enter( &pomp2_region_11, pomp2_ctc_11  );
#pragma omp          for                   nowait
        for (j=0; j<STREAM_ARRAY_SIZE; j++)
            c[j] = a[j];
{ POMP2_Task_handle pomp2_old_task;
...
#pragma omp barrier
  ...
  POMP2_For_exit( &pomp2_region_11 );
 }
...
{   POMP2_For_enter( &pomp2_region_13, pomp2_ctc_13  );
#pragma omp          for                   nowait
	for (j=0; j<STREAM_ARRAY_SIZE; j++)
	    c[j] = a[j]+b[j];
{ POMP2_Task_handle pomp2_old_task;
...
#pragma omp barrier
  ...
  POMP2_For_exit( &pomp2_region_13 );
 }
	...
	ssize_t j;
{
  ...
  int pomp2_if = 1;
  POMP2_Task_handle pomp2_old_task;
  POMP2_Parallel_fork(&pomp2_region_14, pomp2_if, pomp2_num_threads, &pomp2_old_task, pomp2_ctc_14 );
#pragma omp parallel     POMP2_DLIST_00014 firstprivate(pomp2_old_task) if(pomp2_if) num_threads(pomp2_num_threads)
{   POMP2_Parallel_begin( &pomp2_region_14 );
{   POMP2_For_enter( &pomp2_region_14, pomp2_ctc_14  );
#pragma omp          for                   nowait
	for (j=0; j<STREAM_ARRAY_SIZE; j++)
	    a[j] = b[j]+scalar*c[j];
{ POMP2_Task_handle pomp2_old_task;
  POMP2_Implicit_barrier_enter( &pomp2_region_14, &pomp2_old_task );
#pragma omp barrier
  POMP2_Implicit_barrier_exit( &pomp2_region_14, pomp2_old_task ); }
  POMP2_For_exit( &pomp2_region_14 );
 }
  POMP2_Parallel_end( &pomp2_region_14 ); }
  POMP2_Parallel_join( &pomp2_region_14, pomp2_old_task ); }
}
